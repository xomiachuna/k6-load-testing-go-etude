...
func newResource(ctx context.Context) (*sdkresource.Resource, error) {
	...
	if err != nil {
        return nil, fmt.Errorf("new otel sdk resource: %w", err)
	}
...
func newTracerProvider(ctx context.Context, resource *sdkresource.Resource) (*sdktrace.TracerProvider, error) {
	...
	if err != nil {
        return nil, fmt.Errorf("new otel tracer provider: %w", err)
	}
...
func newMeterProvider(ctx context.Context, resource *sdkresource.Resource) (*sdkmetric.MeterProvider, error) {
	...
	if err != nil {
        return nil, fmt.Errorf("new otel meter provider: %w", err)
	}
...
func (lp *scopeInjectingLogProcessor) Shutdown(_ context.Context) error   { 
    return nil
}

func (lp *scopeInjectingLogProcessor) ForceFlush(_ context.Context) error {
    return nil
}

func (lp *scopeInjectingLogProcessor) Enabled(_ context.Context, _ sdklog.EnabledParameters) bool {
    return true
}
...
func newLoggerProvider(ctx context.Context, resource *sdkresource.Resource) (*sdklog.LoggerProvider, error) {
	...
	if err != nil {
        return nil, fmt.Errorf("new otel logger provider: %w", err)
	}
...
// SetupOtelSDK configures OpenTelemetry instrumentation
// To control it, set OTEL_EXPORTER_OTLP_ENDPOINT that points to an exporter
func SetupOtelSDK(ctx context.Context) (shutdownOtel func(ctx context.Context) error, err error) {
	...
	shutdownOtel = func(ctx context.Context) error {
		...
		for i, callback := range doOtelShutdownCallbacks {
			if cbErr := callback(ctx); cbErr != nil {
                shutdownErr = errors.Join(shutdownErr, fmt.Errorf("otel studown callback #%d: %w", i, cbErr))
			}
		}
		return shutdownErr
	}
	...
	if err != nil {
		return nil, errors.Join(err, shutdownOtel(ctx))
	}
